      ...
      <div style={{ backgroundColor: 'var(--color-bg-card)', borderRadius: '12px', border: '1px solid var(--color-border)', padding: '20px', marginBottom: '25px', boxShadow: '0 4px 6px rgba(0,0,0,0.02)' }}>
            ...
            <div key={item.producto.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '15px 0', borderBottom: '1px solid var(--color-border-light)' }}>
              <div>
                <h4 style={{ margin: '0 0 5px 0', color: 'var(--color-text)' }}>{item.producto.nombre}</h4>
                  <span style={{ fontSize: '13px', color: 'var(--color-text-muted)' }}>Presentación: {item.producto.presentacion} | u: ${formatearPrecio(precio)}</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                ...
                <span style={{ fontWeight: 'bold', color: 'var(--color-text)', minWidth: '70px', textAlign: 'right' }}>${formatearPrecio(precio * item.cantidad)}</span>
              </div>
            </div>
          );
        })}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', marginTop: '20px', paddingTop: '15px', borderTop: '2px solid var(--color-border-light)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', width: '100%' }}>
            <button onClick={() => { if (window.confirm('¿Estás seguro de vaciar el carrito? Se perderán todos los productos.')) vaciarCarrito(); }} style={{ color: 'var(--color-danger)', background: 'none', border: 'none', cursor: 'pointer', fontWeight: '500' }}>Vaciar Carrito</button>
            <span style={{ fontSize: '20px', fontWeight: 'bold', color: 'var(--color-navy)' }}>Total: ${formatearPrecio(obtenerTotal())}</span>
          </div>
